...
import sys
sys.path.insert(0, '../')
sys.path.append('/home/v-siyuanyang/workspace/MAT')
import numpy as np
import math
import glob
import pyspng
...
def calculate_metrics(path):
    l1 = sorted(glob.glob(path + '/*.png') + glob.glob(folder1 + '/*.jpg'))
    
    # l1 = l1[:5000]
    l1 = random.sample(l1,1000)
    print('length:', len(l1))
    ...
    with torch.no_grad():
        for i, fpath in enumerate(l1):
            print(i)
            _, name = os.path.split(path)
            name = name.split('.')[0]

            img = read_image(fpath).to(device)
...
if __name__ == '__main__':
    ...
    sd_path = '/home/v-siyuanyang/workspace/drag_diffusion/sd_inpaint_coco/images_2'
    lama_path = "/home/v-siyuanyang/workspace/lama/lama_coco_inpaint_2/images" 
    inst_path = '/home/v-siyuanyang/workspace/inst-inpaint/inst_inpaint_coco/images'
    zits_path = "/home/v-siyuanyang/workspace/ZITS_inpainting/zits_coco/images"

    magic_path = '/home/v-siyuanyang/image'

    # sd   pids 0.1362 udis 0.2838 pids 0.1249 uids 0.2273
    # lama pids 0.1754 uid 0.3136  pids 0.1545 uids 0.2690
    # inst pids 0.1050 uids 0.2246 pids 0.0795 uids 0.1676
    # zits pids 0.2082 uids 0.3222 pids 0.1958 uids 0.2815
    # magic                        pids 0.1522 uid 0.2601
    #

    pids, uids = calculate_metrics(lama_path)
    print('pids: %.4f, uids: %.4f' % (pids, uids))
    with open('fid_pids_uids.txt', 'w') as f:
        f.write('pids: %.4f, uids: %.4f' % (pids, uids))
