...
def calculate_metrics(path1):
    # 同时支持.png和.jpg/.jpeg格式
    l1 = sorted(glob.glob(path1 + '/*.png') + glob.glob(path1 + '/*.jpg') + glob.glob(path1 + '/*.jpeg'))
    # l2 = sorted(glob.glob(path2 + '/*.png') + glob.glob(path2 + '/*.jpg') + glob.glob(path2 + '/*.jpeg'))
    ...
    with torch.no_grad():
        for i, fpath1 in enumerate(l1):
            print(i)
            _, name = os.path.split(fpath1)
            name = name.split('.')[0]

            img = read_image(fpath1).to(device)
            #img2 = read_image(fpath1).to(device)
            img1 = img[:,:,:,0:512]
            img2 = img[:,:,:,512:1024]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--coco_output_path', type=str)
    ...
    #
    pids, uids = calculate_metrics(args.coco_output_path)
