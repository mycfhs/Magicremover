...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu',
    ...
    for batch in tqdm(dataloader):
        if stat=='ori':
            ...
            #batch = batch[:,:,:,1024:1536]
            batch = batch[:,:,:,512:1024]
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
                                    device='cpu', num_workers=1, stat='ori'):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(files, model, batch_size, dims, device, num_workers, stat)
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    #files = random.sample(files,1000)#files[:1857]+files[3000:9000]
    m, s = calculate_activation_statistics(files, model, batch_size,
                                            dims, device, num_workers, stat)
    return m, s
...
def calculate_fid_given_paths(paths, batch_size, device, dims, num_workers=1):
    ...
    m1, s1 = compute_statistics_of_path(paths, model, batch_size,
                                        dims, device, num_workers, stat='ori')

    # pathgt = 'trans_coco/trans/gt'
    # m1, s1 = compute_statistics_of_path(pathgt, model, batch_size,
    #                                     dims, device, num_workers, stat='ori')                             
    m2, s2 = compute_statistics_of_path(paths, model, batch_size,
                                        dims, device, num_workers, stat='inpaint')
...
class CLIP(nn.Module):
    ...
    def calcualte_clip(self,path, coco_labels_file):
        # 读取 JSON 数据  
        ...
        # # 打印映射关系  
        # for id, name in category_id_name_map.items():  
        #     print(f"{id}: {name}")  

        #path = "sd_inpaint_coco/images"                                              #all local 0.0712.  #all global 0.0712. # part global 0.1051.  # another part global  0.1150 # another part local 0.1509  # part local. 0.1304

        #path = "/home/v-siyuanyang/workspace/lama/lama_coco_inpaint_2/images"         #all local 0.0712.  #all global 0.0712. # part global 0.1122  # another part global 0.1215 #another part local 0.1401  #part local 0.1304
...
def main():
    ...
    parser.add_argument('--task', type=str, default = 'fid')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--coco_output_path', type=str, default=None)
    parser.add_argument('--coco_labels_file', type=str, default=None)
    
    
    args = parser.parse_args()    
    #path = 'null_text_embeddings_coco/images'
    #path = 'comodgan_coco'
    #path = 'deepfillv2_coco'
    #path = 'sd_inpaint_coco'
    # path = "instruct_coco"
    path = args.coco_output_path
    ...
    ##### ablation 5k                             1k
    ###      fid     cs         uids              fid      cs      uids
    # lama: 17.33    0.2143     0.1884            47.569  0.2142
    # full  22.415              0.2264            50.859. 0.2195
    # -rc
    # -rp
    # -sg
    # -co
    ...
    ######### calculate clip
        ...
        with torch.no_grad():
            clip = CLIP(device)
            clip_sim = clip.calcualte_clip(path, args.coco_labels_file)
        print(clip_sim)
...
##### type                            fid          clip        u-ids
##### without mask full images.       11.709
##### without mask random 1k images   66.304


#############   clip          fid          u-ids
## comodgan     0.2195        8.3643       0.3224
## deepfillv2   0.2191        10.6176      0.2900
## instructdiff 0.2135        13.38        0.2397