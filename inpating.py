...
class StableDiffusion(nn.Module):
    ...
    def real_image_ldm_stable(
        ...
        #img_path = "inpainting_img/56.png"
        #img_path = "New folder/image/1.jpg"
        img_path = "3.png"
        ...
        torch.save(uncond_embeds_list,'inpainting_img/21.pt')
        uncond_embeds_list = torch.load('inpainting_img/21.pt')
        print('done')
        ...
        mask = normalize(mask)        
        self.mask=mask
        self.mask_0 =mask.gt(0.3).to(torch.float32)
        mask =  torch.nn.functional.interpolate(mask.unsqueeze(0),(512,512),mode='bilinear', align_corners=False).squeeze(0)
        torchvision.utils.save_image(torch.concat((img+mask.unsqueeze(1),mask.unsqueeze(1).repeat(1,3,1,1)),dim=-1), 'mask_1.png')
        ...
        for i, t in enumerate(self.scheduler.timesteps):
            ...
            if t > guidance_step:
                ...
                #if t in [t_idx,  t_idx-50, t_idx-100,t_idx-150, t_idx-200]: # or t < 100:
                # if t in [t_idx]: # or t < 100:
                # #if t > 650: # or t < 100:
                #     # num_inner = int(20 * (t-700)/(t_idx-700)) + 3
                #     # for iter in range(num_inner):
                ...
                plus_guidance = False
                self.save_attention.self_attn = True #False
                latents_edit = self.diffusion_step(latents_edit, context, t, guidance_scale,  low_resource, plus_guidance=plus_guidance)
                self.save_attention.set_empty_store() 
        ...
        img_edit = self.decode_latents(latents_edit)
        torchvision.utils.save_image(torch.concat((img_ori,img_edit),dim=0),'1.png')
        torchvision.utils.save_image( img_edit,'2.png')
    ...
    def diffusion_step(self, latents, context, t, guidance_scale,  low_resource=False, plus_guidance=False, remain_step=False, latents_ori=None):
        #with torch.no_grad():
        if plus_guidance:
            ...
            with torch.enable_grad():
                ...
                attn_edit_thresh = torch.nn.functional.interpolate(attn_edit_thresh.unsqueeze(0).unsqueeze(0),(64,64),mode='bilinear', align_corners=False).squeeze(0).squeeze(0)
                # loss.backward()
                ...
                torchvision.utils.save_image(torch.concat((normalize(attn_edit_thresh).unsqueeze(0),normalize(gradients.sum(1)), normalize(gradients.sum(1)) *  attn_edit_thresh, self.mask),dim=2),f'attn_thresh/{t}.png')
    ...
    def guidance(self, t, latents, latents_ori):
        ...
        attn_edit_obj  = 1
        for obj_id in  object_id_d:
            #attn_edit_obj = attn_edit_obj * (attn_edit[obj_id] / attn_edit[obj_id].max())
            attn_edit_obj = attn_edit_obj * attn_edit[obj_id]
        thresh = 0.2
        ...
        grad_mask = 1
        for i in range(attn_edit_obj.shape[0]):
            grad_mask = grad_mask * normalize(attn_edit_obj[i])
        
        
        # thresh_1 = max(min(0.2, t / 1000), 0.2)
        # thresh_2 = max(min(0.5, t / 1000), 0.5)
        
        #attn_edit_thresh = self.sigmod(100 * (normalize(attn_edit_obj) - thresh)) 
        # attn_edit_thresh = self.sigmod(100 * (normalize(attn_edit_obj) - thresh_1)) * self.mask + self.sigmod(100 * (normalize(attn_edit_obj) - thresh_2)) * (1-self.mask)

        #normalize(attn_edit_obj)
        
        #loss = self.mse(attn_edit_obj, torch.ones_like(attn_edit_obj,device=attn_edit_obj.device) * attn_edit_obj.min()) * 5 
        # mask = torch.zeros_like(attn_edit_obj,device=attn_edit_obj.device)
        #mask[:,202:506,222:375] = 1 # woman 2
        #mask[:,3:293,0:188] = 1 # tree 1
        #mask[:,226:359,267:508] = 1
        # mask[:,64:512,236:451] = 1
        attn_edit_obj = attn_edit_obj 
        
        # tmp = torch.quantile(attn_edit_obj.flatten(start_dim=1), 0.1, dim=1, keepdim=False)
        ...
        mask =  torch.ones_like(attn_edit_obj, device=attn_edit_obj.device)
        ...
        attn_edit_obj = attn_edit_obj   / self.init_max  * 10
        ...
        lmbda = max(min((t.to(self.device)-300)/600-0.2, 0.8),0.3)
        tmp = torch.quantile(attn_edit_obj.flatten(start_dim=1),  0., dim=1, keepdim=False).item()
        # attn_edit_obj = torch.where(
        #     attn_edit_obj >= tmp
        #     , attn_edit_obj
        #     , torch.zeros_like(attn_edit_obj)
        ...
        loss = self.mse( attn_edit_obj * mask, mask * attn_min) * 10
        #loss = self.mse( attn_edit_obj * mask, mask * 0) * 5
        
        # tmp = torch.quantile(attn_edit_obj.flatten(start_dim=1),  0.6, dim=1, keepdim=False).item()
        # # attn_edit_obj = torch.where(
        # #     attn_edit_obj >= tmp
        # #     , attn_edit_obj
        # #     , torch.zeros_like(attn_edit_obj)
...
if __name__=='__main__':
    ...
    #prompts = ["a photo of a puppy playing by a creek with its reflection cast in the water"]
 
    prompts = ["A photo of a woman ."] #["A photo of a woman"]
    ...
    images, x_t = sd.real_image_ldm_stable(prompts, latent=None, num_inference_steps=50, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=11, t_ratio=0.9)
