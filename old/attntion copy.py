...
class StableDiffusion(nn.Module):
    ...
    def diffusion_step(self, latents, context, t, guidance_scale,  low_resource=False, plus_guidance=False, save_noise=True):
        ...
        if plus_guidance:
            ...
            with torch.enable_grad():
                ...
                noise_pred = noise_pred + gradients *  2
    ...
    def guidance(self, t):
        ...
        #loss = (self.l1_loss(attn_ori_thresh_s, attn_edit_thresh_s)*30 + self.l1_loss(a_edit,a_ori)*30)/len(object_id_s)
        ...
        attn_ori_thresh = normalize(self.sigmod(10*(normalize(attn_ori_obj)-0.5)))
        
        attn_edit_obj = attn_edit[object_id_d]
        attn_edit_thresh = normalize(self.sigmod(10*(normalize(attn_edit_obj)-0.5)))
        ...
        ori_cen = tensor_centroid(attn_ori_thresh)
        target_cen=15,15


        
        loss = self.l1_loss(a_edit,a_ori)
        loss +=  (torch.abs(ori_cen[0] + 10 - cur_cen[0] ) + torch.abs(ori_cen[1] - cur_cen[1]))*5
        loss += torch.abs(edit_size-ori_size) * 10
        
        #loss += (self.l1_loss(attn_ori[:object_id], attn_edit[:object_id]) + self.l1_loss(attn_ori[object_id+1:], attn_edit[object_id+1:])) * 1#* 50
        loss += self.l1_loss(attn_ori_thresh_s, attn_edit_thresh_s) * 10
...
if __name__=='__main__':
    ...
    prompts = ['a photo of a boombox on a camel near a pond']
    #[ "a photo of a dog and a ball on the grass"]
    #while True:
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=50, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=701)
