...
class Save_Attention(nn.Module):
    ...
    def save_self_attn(self, k, v, is_cross, place_in_unet):
        ...
        if is_cross==False:
            ...
            #if attn.shape[1] <= 32 ** 2:  # avoid memory overhead
            self.step_store[key]['k'].append(k)
            self.step_store[key]['v'].append(v)
            ...
            def forward(x, encoder_hidden_states=None, attention_mask=None):
                ...
                if is_cross==False:
                    if save_attention.self_attn==True:
                        k = save_attention.attention_all_step[f'{save_attention.cur_t}'][place_in_unet+'_self']['k'][save_attention.place_in_cross]
                        v = save_attention.attention_all_step[f'{save_attention.cur_t}'][place_in_unet+'_self']['v'][save_attention.place_in_cross]
                        
                        if save_attention.place_in_cross < (len(save_attention.attention_all_step[f'{save_attention.cur_t}'][place_in_unet+'_self']['k'])-1):
...
if __name__=='__main__':
    ...
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=100, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=201)
