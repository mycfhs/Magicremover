...
class StableDiffusion(nn.Module):
        ...
        def unet_forward(self):
            def forward(
                ...
                if self.conv_norm_out:
                    ...
                    save_attention.phi.append(sample)
    ...
    def guidance(self, t):
        ...
        target_cen=torch.tensor([30,10],device=attn_edit.device)


        #torch.abs(target_cen[0]  - cur_cen[0] )
        loss =  (torch.abs(target_cen[0]  - cur_cen[0]) + torch.abs(target_cen[1]  - cur_cen[1]))*50
        #loss = self.l1_loss(target_cen[0], cur_cen[0]) + self.l1_loss(target_cen[1], cur_cen[1])       
        
        #loss += self.l1_loss(edit_size,ori_size)  * 10000
        ...
        #loss += self.l1_loss(a_edit,a_ori)  * 10000
...
if __name__=='__main__':
    ...
    #prompts = ["a photo of a hamburger and an ice cream"]
    ...
    # dog_attn= torch.tensor(np.array(dog_attn)).permute(2,0,1)[0]/255
    # dog_attn[:,10:250] = dog_attn[:,250:490]
    # dog_attn[:,250:490]=0
    
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=200, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=101)
