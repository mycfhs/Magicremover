...
class StableDiffusion(nn.Module):
    ...
    def text2image_ldm_stable(
        ...
        context = [text_embeddings, text_embeddings] #[uncond_embeddings, text_embeddings]
        if not low_resource:
        ...
        latents1 = latents.clone().detach().requires_grad_(True)
        with torch.enable_grad():
            latents = torch.concat((latents, latents1),dim=0) 
        ...
        torchvision.utils.save_image(img,'1.png')
        ...
        def ca_forward(self, place_in_unet):
            ...
            def forward(x, encoder_hidden_states=None, attention_mask=None):
                ...
                if is_cross is False:
                    mask = torch.ones_like(k,device=k.device).requires_grad_(False)
                    mask[k.shape[0]//2:]=0
                    k = k * mask + k[:k.shape[0]//2].repeat(2,1,1) * (1-mask)
                    v = v * mask + v[:v.shape[0]//2].repeat(2,1,1) * (1-mask)
    ...
    def diffusion_step(self, latents, context, t, guidance_scale,  low_resource=False, plus_guidance=False, save_noise=True):
        ...
        if plus_guidance:
            with torch.enable_grad():
                ...
                #loss.backward()
                #gradients = latents.grad[latents.grad.shape[0]//2:]
                gradients = torch.autograd.grad(loss, latents_input, retain_graph=False, create_graph=False, allow_unused=True)[0] 
                #gradients = torch.autograd.grad(loss, latents[latents.shape[0]//2:], retain_graph=False, create_graph=False, allow_unused=True)[0] 
                print(f'time_step:{t}, loss:{loss}, gradients_max:{gradients.max()}')
                noise_pred[noise_pred.shape[0]//2:] = noise_pred[noise_pred.shape[0]//2:] + gradients[noise_pred.shape[0]//2:] #*  50
        
        else:
            latents_input = latents.clone().detach()
            noise_pred = self.unet(latents_input, t, encoder_hidden_states=context)["sample"]
...
if __name__=='__main__':
    ...
    x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=200, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=201)
