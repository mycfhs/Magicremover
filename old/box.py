...
class StableDiffusion(nn.Module):
    ...
    def guidance(self, t):
        ...
        mask_ori = torch.tensor(np.array(mask_ori),device=device).permute(2,0,1).unsqueeze(0)/255
        mask_ori = torch.nn.functional.interpolate(mask_ori, size=(512, 512), mode='bilinear', align_corners=False)
        mask_ori = mask_ori[:,0,:,:]
        
        
        # mask_path = "masks/edit2.png"
        # mask_edit = Image.open(mask_path).convert('RGB')
        # mask_edit = torch.tensor(np.array(mask_edit),device=device).permute(2,0,1).unsqueeze(0)/255
        # mask_edit = torch.nn.functional.interpolate(mask_edit, size=(512, 512), mode='bilinear', align_corners=False)
        # mask_edit = mask_edit[:,0,:,:]
        
        r1,r2,c1,c2 = find_bounding_box(mask_ori.squeeze(0))
        re1,re2,ce1,ce2 = 45,252,379,485
        tar_attn = torch.nn.functional.interpolate(attn_edit_obj[:,re1:re2,ce1:ce2].unsqueeze(0),(r2-r1,c2-c1),mode='bilinear', align_corners=False )
        
        loss = self.l1_loss(attn_ori_obj[:,r1:r2,c1:c2] * mask_ori[:,r1:r2,c1:c2], tar_attn * mask_ori[:,r1:r2,c1:c2]) * 20
        
        
        
        tar_phi = torch.nn.functional.interpolate(phi_edit[:,:,re1:re2,ce1:ce2],(r2-r1,c2-c1),mode='bilinear', align_corners=False )
        loss += self.l1_loss(phi_ori[:,:,r1:r2,c1:c2] * mask_ori[:,r1:r2,c1:c2], tar_phi * mask_ori[:,r1:r2,c1:c2]) * 5
...
if __name__=='__main__':
    ...
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=100, guidance_scale=1, generator=None, low_resource=False,  guidance_step=101,t_ratio=0.8)
