...
class StableDiffusion(nn.Module):
    ...
    def drag(
        ...
        #img_path = "2.png" 
        #img_path = "dreambooth/dataset/vase/00.jpg" 
        img_path = "dreambooth/dataset/can/01.jpg" 
        ...
        latent, init_latents = self.init_latent(latent, height, width, generator, batch_size)
        latents_ori=init_latents.clone().detach()
        latents_edit=init_latents
    ...
    def text2image_se_ldm_stable(
        ...
        se_embeddings = self.text_encoder(se_input.input_ids.to(self.device))[0]
        
        
        context = [uncond_embeddings, text_embeddings, se_embeddings]
        if not low_resource:
            context = torch.cat(context)
        ...
        for t in self.scheduler.timesteps:
            self.save_attention.state = "ori"
            self.save_attention.self_attn = False
            latents_ori = self.diffusion_step2(latents_ori, context, t, guidance_scale)
            #self.save_attention.all_store(t)
            ...
            if t > guidance_step:
                ...
                if t > 701:
                    for iter in range(1):
                        latents_edit = self.diffusion_step2(latents_edit, context, t, guidance_scale, plus_guidance=plus_guidance, remain_step=True)
                        self.save_attention.set_edit_empty_store()

                latents_edit = self.diffusion_step2(latents_edit, context, t, guidance_scale,  plus_guidance=plus_guidance)
                self.save_attention.set_empty_store()
            else:
                plus_guidance = False
                self.save_attention.self_attn = False
        ...
        def unet_forward(self):
            def forward(
                ...
                for i, upsample_block in enumerate(self.up_blocks):
                    ...
                    if i in [1,2,3]:
                        if i==1:
                            tmp = torch.nn.functional.interpolate(sample, size=(512, 512), mode='bilinear', align_corners=False)  * 0.1
                        if i == 2:
                            tmp = torch.nn.functional.interpolate(sample, size=(512, 512), mode='bilinear', align_corners=False) * 0.1
                        if i == 3:
    ...
    def diffusion_step2(self, latents, context, t, guidance_scale,  plus_guidance=False, remain_step=False):
        #with torch.no_grad():
        if plus_guidance:
            latents.requires_grad_(True)
            with torch.enable_grad():
                latents_input = torch.cat([latents] * 2)
                noise_pred = self.unet(latents_input, t, encoder_hidden_states=context[:2])["sample"]
                noise_pred_uncond, noise_prediction_text = noise_pred.chunk(2)
                ...
                noise_pred_se = self.unet(latents, t, encoder_hidden_states = context[2])["sample"]

                noise_pred_edit = noise_pred + 1 * (noise_pred_se - noise_pred_uncond)
    ...
    def guidance(self, t):
        ...
        #### object size
        ori_size = torch.sum(attn_ori_thresh)
        edit_size = torch.sum(attn_edit_thresh) 
        ...
        #loss =  torch.abs(target_cen[0]  - cur_cen[0]) * 5
        
        #loss =  torch.abs(target_cen[1]  - cur_cen[1]) * 5
        #loss = self.l1_loss(target_cen[0], cur_cen[0]) + self.l1_loss(target_cen[1], cur_cen[1])       
        
        #loss += self.l1_loss(edit_size,ori_size)  * 0.09

        #loss += self.l1_loss(attn_ori_thresh_s, attn_edit_thresh_s)*20
        
        
        ### appearance   
        phi_edit = self.save_attention.phi['edit'][1:]
        a_edit = ((phi_edit * attn_edit_thresh.repeat(1,phi_edit.shape[1],1,1)).sum(-1).sum(-1)) 
        
        phi_ori = self.save_attention.phi['ori'][1:]
        a_ori = ((phi_ori * attn_ori_thresh.repeat(1,phi_edit.shape[1],1,1)).sum(-1).sum(-1)) 
        
        #loss += self.l1_loss(a_edit,a_ori)  * 10
        
                
        mask_ori_path = "masks/dog.png"
        mask_edit_path = "masks/edit2.png"
        mask_ori = Image.open(mask_ori_path).convert('RGB')
        mask_ori= torch.tensor(np.array(mask_ori),device=self.device).permute(2,0,1)[0]/255
        mask_ori = torch.nn.functional.interpolate(mask_ori.unsqueeze(0).unsqueeze(0),(64,64),mode='bilinear', align_corners=False).squeeze(0).squeeze(0)
        #ori_f = mask_ori * phi_ori 
        nonzero_indices_ori = torch.nonzero(mask_ori)
        
        
        mask_edit = Image.open(mask_edit_path).convert('RGB')
        mask_edit= torch.tensor(np.array(mask_edit),device=self.device).permute(2,0,1)[0]/255
        mask_edit = torch.nn.functional.interpolate(mask_edit.unsqueeze(0).unsqueeze(0),(64,64),mode='bilinear', align_corners=False).squeeze(0).squeeze(0)
        #edit_f = mask_edit * phi_edit
        nonzero_indices_edit = torch.nonzero(mask_edit)
        
        
        ori_shape_flatten = phi_ori[:,:, nonzero_indices_ori[:, 0], nonzero_indices_ori[:, 1]]
        edit_shape_flatten = phi_edit[:,:, nonzero_indices_edit[:, 0], nonzero_indices_edit[:, 1]]
        
        size=max(nonzero_indices_ori.shape[0],nonzero_indices_edit.shape[0])
        if ori_shape_flatten.shape[-1]<size:
           ori_shape_flatten =  torch.nn.functional.interpolate(ori_shape_flatten, size=size, mode='linear', align_corners=False)  
        if edit_shape_flatten.shape[-1]<size:
           edit_shape_flatten =  torch.nn.functional.interpolate(edit_shape_flatten, size=size, mode='linear', align_corners=False)  
           
        
        #loss += self.l1_loss(ori_shape_flatten, edit_shape_flatten) * 200  #+ self.l1_loss(attn_edit_thresh*mask_edit, torch.zeros_like(attn_edit_thresh,device=attn_edit_thresh.device)) * 1

        #loss = (1-torch.nn.functional.cosine_similarity((mask_edit*attn_edit_obj).flatten(), (mask_ori * attn_ori_obj).flatten(), dim=0)) * 100

       
        loss =  self.l1_loss(ori_shape_flatten, edit_shape_flatten) * 1 + self.l1_loss(attn_edit_obj, mask_edit) * 100 #+ self.l1_loss(attn_edit_thresh*mask_edit, torch.zeros_like(attn_edit_thresh,device=attn_edit_thresh.device)) * 1
        #loss = self.l1_loss(attn_edit_thresh, torch.zeros_like(attn_edit_thresh,device=attn_edit_thresh.device)) * 10. 
    ...
    def guidance2(self, t):
       
        ### appearance   
        phi_edit = self.save_attention.phi['edit']
        phi_edit = torch.nn.functional.interpolate(phi_edit, size=(512, 512), mode='bilinear', align_corners=False)
        phi_ori = self.save_attention.phi['ori']
        phi_ori = torch.nn.functional.interpolate(phi_ori, size=(512, 512), mode='bilinear', align_corners=False)
        ...
        return loss * 3 * ((1-self.scheduler.alphas_cumprod[t]) ** 0.5)
...
if __name__=='__main__':
    ...
    #prompts = ["a photo of a dog sitting in the street"]
    prompts = ["a photo of a beer can"]
    ...
    #images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=50, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=101)
    images, x_t = sd.drag(prompts, latent=None, num_inference_steps=100, guidance_scale=1, generator=None, low_resource=False,  guidance_step=101, t_ratio=0.8)
    #images, x_t = sd.text2image_se_ldm_stable(prompts, latent=None, num_inference_steps=100, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=101)
