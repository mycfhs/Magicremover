...
class StableDiffusion(nn.Module):
    ...
    def real_image_ldm_stable(
        ...
        #img_path = "8.jpg"
        img_path = "inpainting_img/11.png"
        ...
        torch.save(uncond_embeds_list,'inpainting_img/11.pt')
        #uncond_embeds_list = torch.load('inpainting_img/11.pt')
        print('done')
    ...
    def guidance(self, t):
        ...
        attn_edit_obj_1 = attn_edit[5]
        attn_edit_obj_1 = torch.nn.functional.interpolate(attn_edit_obj_1.unsqueeze(0).unsqueeze(0),(512,512),mode='bilinear', align_corners=False).squeeze(0)
        
        attn_edit_obj_2 = attn_edit[8]
        attn_edit_obj_2 = torch.nn.functional.interpolate(attn_edit_obj_2.unsqueeze(0).unsqueeze(0),(512,512),mode='bilinear', align_corners=False).squeeze(0)
        
        attn_edit_obj_3 = attn_edit[10]
        attn_edit_obj_3 = torch.nn.functional.interpolate(attn_edit_obj_3.unsqueeze(0).unsqueeze(0),(512,512),mode='bilinear', align_corners=False).squeeze(0)
        #attn_edit_thresh = normalize(self.sigmod(1*(normalize(attn_edit_obj)-0.5)))
        thresh = max(min(0.5, t / 1000), 0.3)
        
        attn_edit_thresh_1 = self.sigmod(100 * (normalize(attn_edit_obj_1) - thresh))
        attn_edit_thresh_2 = self.sigmod(100 * (normalize(attn_edit_obj_2) - thresh))
        attn_edit_thresh_3 = self.sigmod(100 * (normalize(attn_edit_obj_3) - thresh))
        ...
        #loss = (self.mse(attn_edit_thresh_1, mask_dog)+self.mse(attn_edit_thresh_2, mask_tree)+self.mse(attn_edit_thresh_3, mask_grass)) * 50
        loss = self.mse(attn_edit_obj_1, mask_dog) * 80
...
if __name__=='__main__':
    ...
    sd = StableDiffusion(device, fp16=False, vram_O=False, sd_version='2.1',object_id_d=[6,8,10], object_id_s=[8])
    ...
    prompts = ["a photo of a puppy"] #, tree and grass"]
    ...
    # mask_path = "masks/dog/dog.png"
    # mask = Image.open(mask_path).convert('RGB')
    # mask = torch.tensor(np.array(mask),device=device).permute(2,0,1).unsqueeze(0)/255
    # mask_dog = torch.nn.functional.interpolate(mask, size=(512, 512), mode='bilinear', align_corners=False)
    # mask_dog = mask_dog[:,0,:,:]
    ...
    #images, x_t = sd.real_image_ldm_stable(prompts, latent=None, num_inference_steps=100, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=1, t_ratio=0.85)
