...
class StableDiffusion(nn.Module):
            ...
            def forward(
                ...
                for i, upsample_block in enumerate(self.up_blocks):
                    ...
                    if i in [1,2,3]:
                        ...
                        if i == 2:
                            tmp = torch.nn.functional.interpolate(sample, size=(512, 512), mode='bilinear', align_corners=False) * 1.2
                        if i == 3:
                            tmp = torch.nn.functional.interpolate(sample, size=(512, 512), mode='bilinear', align_corners=False) * 1
    ...
    def guidance(self, t):
        ...
        target_cen=torch.tensor([350,200],device=attn_edit.device)
        ...
        loss += self.l1_loss(edit_size,ori_size)  * 50
       
        
        ### appearance   
        phi_edit = self.save_attention.phi['edit'][1:]
        a_edit = ((phi_edit * attn_edit_thresh.repeat(1,phi_edit.shape[1],1,1)).sum(-1).sum(-1)) / torch.sum(mask_edit)
        
        phi_ori = self.save_attention.phi['ori'][1:]
        a_ori = ((phi_ori * attn_ori_thresh.repeat(1,phi_edit.shape[1],1,1)).sum(-1).sum(-1)) / torch.sum(mask_ori)
        
        
        loss += self.l1_loss(a_edit,a_ori)  * 5
...
if __name__=='__main__':
    ...
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=100, guidance_scale=1, generator=None, low_resource=False,  guidance_step=101, t_ratio=0.75)
