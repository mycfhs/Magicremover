...
class StableDiffusion(nn.Module):
    ...
    def guidance(self, t):
        ...
        # ori_a = phi_ori.sum(1) * attn_ori_obj
        # edit_a = phi_edit.sum(1) * attn_edit_obj
        # ori_a = torch.nn.functional.interpolate(ori_a.unsqueeze(0), (512,512),mode='bilinear', align_corners=False)
        # edit_a = torch.nn.functional.interpolate(edit_a.unsqueeze(0), (512,512),mode='bilinear', align_corners=False)
        # loss = 0
        # for qi in neighbor(int(cur_point[0]), int(cur_point[1]), 3):
        #     f1 = bilinear_interpolate_torch(ori_a, qi[0], qi[1]).detach()
        #     f2 = bilinear_interpolate_torch(edit_a, qi[0] + d[0], qi[1] + d[1])
        #     loss += self.l1_loss(f2, f1)
        
        # mask = torch.ones_like(ori_a,device=ori_a.device)
        # #mask[:,:,96:367,80:383]=0
        # mask[:,:,39:240,128:349]=0
        # loss += self.l1_loss(ori_a*mask, edit_a*mask) * 10
        
        
        loss = self.l1_loss(a_edit,a_ori) * 0.01

        loss = loss * ((1-self.scheduler.alphas_cumprod[t]) ** 0.5) # * 3
...
if __name__=='__main__':
    ...
    images, x_t = sd.text2image_ldm_stable(prompts, latent=None, num_inference_steps=50, guidance_scale=7.5, generator=None, low_resource=False,  guidance_step=201)
