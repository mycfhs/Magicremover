...
class StableDiffusion(nn.Module):
    ...
    def drag_diffusion(self, img, point, prompts, negative_prompts='', height=512, width=512, num_inference_steps=50, guidance_scale=7.5):
        ...
        ### reconstrut loss on x_t_1x_t_512_ori = F.interpolate(ddim_latent[ddim_idx], size=(512, 512), mode='bilinear', align_corners=False)
        ...
        img_gt = self.aug(img)
        image_z_gt = self.clip_model.encode_image(img_gt)
        image_z_gt = image_z_gt / image_z_gt.norm(dim=-1, keepdim=True)
        ...
        while(torch.norm((p_t-p_c).type(torch.float))>1):
            ...
            with torch.enable_grad():
                ...
                loss += self.l1_loss(x_t_pre_512  * mask, x_t_1_512  * mask) * 10000   #.1
                
                x_0_pred = self.predict_start(noise_pred, t_idx, x_t)
                img_pred = self.decode_latents(x_0_pred)
                img_pred = self.aug(img_pred)
                image_z_pred = self.clip_model.encode_image(img_pred)
                image_z_pred = image_z_pred / image_z_pred.norm(dim=-1, keepdim=True) # normalize features
                
                loss +=  (1-(image_z_gt * image_z_pred).sum(-1).mean()) * 0.1
            ...
            minx = 1e9
            miny = 1e9
            for qi in neighbor(int(p_c[0]), int(p_c[1]), 4):
            ...
            video = torch.concat((video,dec_img), dim=0)
            print(f'step:{step}, loss:{loss},  target point:{p_t}, current point: {p_c}')
            step+=1 
        torchvision.io.write_video('1.mp4', video.permute(0,2,3,1).cpu()*255, fps=12)
        #############################       
